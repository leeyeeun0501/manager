...
export default function RoomManageEditPage() {
  ...
  }, [building, floor, svgRaw, svgNodes, svgCategories, pendingNodes, pendingCategories, deletedNodes, deletedCategories, showToast])
  ...
  const memoizedMapContent = useMemo(() => {
    ...
    return (
      <div
        ...
        style={{ width: svgViewBox.width, height: svgViewBox.height, transform: `translate(${offsetX}px, ${offsetY}px) scale(${scale})`, cursor: isAddingMode || isAddingCategoryMode ? "crosshair" : "default" }}
      >
        ...
        {pendingCategories.map((category, index) => (
          <div key={`pending-category-${category.id}-${index}`} className={styles.editPagePendingCategoryOverlay} style={{ left: `${category.x - category.width / 2}px`, top: `${category.y - category.height / 2}px`, width: `${category.width}px`, height: `${category.height}px`, borderRadius: category.element === "rect" ? "4px" : "50%" }} title={`새 카테고리: ${categoryNameMap[category.categoryType] || category.categoryType}`} />
        ))}
      </div>
    )
  }, [svgRaw, canvasSize, svgViewBox, svgNodes, deletedNodes, pendingNodes, svgCategories, deletedCategories, pendingCategories, handleMapClick, handleNodeClick, handleCategoryClick, isAddingMode, isAddingCategoryMode, categoryNameMap])
      ...
      <div className={styles["room-content"]}>
        <div className={styles["room-manage-map-wrap"]} style={{ position: "relative", alignItems: 'center' }}>
          ...
          <div className={styles.editPageCanvasContainer} style={{ width: canvasSize.width, height: canvasSize.height }}>
            ...
            {!loading && building && floor && memoizedMapContent}
          </div>
        </div>
      </div>
