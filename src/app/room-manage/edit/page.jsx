...
export default function RoomManageEditPage() {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`노드/카테고리 추가 실패: ${errorData.message || "알 수 없는 오류"}`)
      }
    } catch (error) {
  ...
  const addNodesToSvg = (svgXml, nodes, categories = []) => {
    ...
    // 각 카테고리를 SVG 요소로 추가 (카테고리는 빨간색으로 구분)
    categories.forEach(category => {
      ...
      circle.setAttribute("r", "2.1957438") // 기존 노드와 동일한 반지름
      circle.setAttribute("style", "fill:#ff6b6b;fill-opacity:0;stroke:#ff6b6b;stroke-width:1") // 카테고리는 빨간색으로 구분
